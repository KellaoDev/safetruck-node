...
const { checklists, getReturnChecklists, returnChecklist, loading, error } = useChecklist()
const userName = ref('')
...
const returned = async (id) => {
  await returnChecklist(id)
  await getReturnChecklists()
}
...
<template>
  <div class="dashboard">
    <div class="card">
      <h2>Seus Checklists de Retorno</h2>
    </div>
    ...
    <div v-else>
      <ul>
        <li v-for="item in filteredChecklists" :key="item.id" class="checklist-item">
          ...
          <p><strong>Status:</strong> {{ item.status === 'released' ? 'Aguardando retorno' : item.status }}</p>

          <p><strong>Data de liberação:</strong> {{ new Date(item.date_released).toLocaleString() }}</p>

          <button class="button" v-if="item.status === 'released'" @click="returned(item.id)">
            Retornar Checklist
          </button>
        </li>
      </ul>
